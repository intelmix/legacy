...
def manage_api(request):
    ...
    if func == 'SignUp':
        ...
        pass
    elif func == 'ToggleStar':
        return toggle_star(request)
    elif func == 'GetUserProfile':
        return get_user_profile(request)
    elif func == 'SetUserProfile':
        return set_user_profile(request)
    elif func== 'GetAllSources':
        return get_all_sources(request)
    elif func == 'GetBulletinDialog':
        return get_bulletin_dialog(request)
    elif func == 'AddBulletin':
        return add_bulletin(request)
    elif func == 'GetUserBulletin':
        return get_user_bulletin(request)
    elif func == 'AddBulletinFilter':
        return add_bulletin_filter(request)
    elif func == 'GetBulletinFilterDialog':
        return get_bulletin_filter_dialog(request)
    elif func == 'GetBulletinFilter':
        return get_bulletin_filter(request)
    elif func == 'RemoveBulletinFilter':
        return remove_bulletin_filter(request)
    elif func == 'RemoveBulletin':
        return remove_bulletin(request)
    elif func == 'SourceManager':
...
def sign_up(request):
    try:
        ...
        if request.POST.has_key('username') and request.POST.has_key('password') :
            Setting.log.info("signup attemp for  username:"+request.POST['username'])
            ...
            if ( captcha_result == 'pass' ):
                uw = UserWorker()
                init_db()
                if uw.sign_up(request.POST['username'], request.POST['password']):
                    success = True
                    Setting.log.info("signup done for email:  username:"+request.POST['username'])
                else:
                    fail_type = 'data'
...
def remove_bulletin_filter(request):
    init_db()
    nw = NewsWorker()
    res = nw.remove_bulletin_filter(request.POST['filter_id'],request.session['user_id'])
    return {'success':res}
