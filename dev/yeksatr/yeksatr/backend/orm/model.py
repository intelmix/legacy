...
class     User(BaseModel):
    ...
    reset_password_key = CharField(max_length=32, null=True)
...
class     BulletinFilter(BaseModel):
    fk_bulletin = ForeignKeyField(db_column='fk_bulletin_id', rel_model=    Bulletin, to_field='id')
    fk_source = ForeignKeyField(db_column='fk_source_id', null=True, rel_model=    Source, to_field='id')
    query_text = CharField(max_length=100, null=True)
    scan_period = IntegerField(null=True)
    starred = IntegerField(null=True)

    class Meta:
        db_table = 'tbl_bulletin_filter'
...
class VwNewsCount(BaseModel):
    news_count = BigIntegerField()
    title = CharField(max_length=45)

    class Meta:
        db_table = 'vw_news_count'
