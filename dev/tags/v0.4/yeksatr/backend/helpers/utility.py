...
def GetRow(str):
    return "<td>"+EncodeToUtf8(str)+"</td>"

def GetInputValue(form,key):
...
def PrettyDate1(seconds):
    return u"%d ثانیه پیش" % (33)
def PrettyDate(seconds):
    a_min = 60
    a_hour = 60*a_min
    a_day = 24 * a_hour
    a_week = a_day*7
...
def GetData(orig_url):
    ...
    try:
        ...
        Setting.error_log.WriteLog( 'fetching url with quoting : ' + ex.__str__()  )
        #print( ex.__str__()+"  ")
    ...
    try:              
        ...
        Setting.error_log.WriteLog( 'fetching url without quoting : ' + ex.__str__()  )
        #print( ex.__str__()+"  ")
